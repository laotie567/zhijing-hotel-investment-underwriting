...
- 将 macOS/Mac Mini 的标准采集路径固化为 Playwright 全量 2km 地图候选池加 Ego Lite OTA 深调；Ui.Vision+OpenCLI 携程实时价 Worker 保留为显式启用的 P2 扩展，不再是默认部署前置。
- Ego Lite 现将结构化可订房型卡片输出为带来源 ID、URL 和采集时间的 `room_type_evidence`，并写入 HTML 和飞书“竞品报价与视觉证据”的“房型观察”记录。它明确不是报价或 ADR 样本。
- 修复页面正文宽泛匹配将酒店简介、特色标签或评论误写为房型的问题。页面明确无房且没有结构化房型卡时返回可见房型待补；不会编造房型，也不会用附近酒店的价格替代目标物业。
- 明确 `NO_INVENTORY` 为已完成的负向价格事实：Ego 保留页面来源和已取得的公开图片，ADR 保持为空。

### Delivery and validation

- 飞书 manifest 升为 `1.2`，`竞品报价与视觉证据.证据类型` 新增“房型观察”；宿主迁移模板时只需增补此选项，保留既有字段和选项。
- 实测已登录的 Ego Lite 携程详情页：可订页面取得结构化房型、P1 页面价格和图片；无房页面正确返回 `NO_INVENTORY` 及房型待补。两类页面均不依赖 Codex Computer Use。
