...
当前开发版本为 `1.0.0`。生产发布物只有
`hotel-investment-underwriting/`，其包内 `VERSION` 是唯一版本来源：

| 项目 | 当前状态 |
|---|---|
| 单一 Skill 入口 | `hotel-investment-underwriting/1.0.0` |
| 财务计算器 | 3.1.0，保留并由 Golden Master 锁定 |
| 财务输入契约 | `project-input.schema.json` |
| 2km 竞品请求契约 | `skill-request.schema.json` |
| 页面市场证据契约 | `market-evidence-collection/v2`；全量地图 Playwright Profile + Ego Lite OTA Profile + Ui.Vision/OpenCLI 携程实时价 Profile，Kimi WebBridge/crawl4ai/xcrawl 可替换 |
