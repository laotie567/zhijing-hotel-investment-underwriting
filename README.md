...
当前开发版本：`0.12.0`。这是一个用于签约前判断的单一生产 Skill，不是独立 App、审批系统或项目数据库。它包含一个无界面的、可替换引擎的页面市场证据采集工具。
...
只发布 `hotel-investment-underwriting/`。宿主负责对话、来源账户/密钥、持久化和消息发送；Skill 的 `collect_market_evidence` 通过 Playwright、Ego Lite、Kimi WebBridge、crawl4ai、xcrawl 或 OpenCLI 采集页面证据，但不拥有 UI、登录凭证、数据库或状态系统。
...
- 页面证据采集器必须先完成全量 2km 候选，再对明确价格/视觉标杆采集房型、图片和报价；回执标明所用引擎、Profile、每个页面来源 URL/HTTP 状态、采集时间和五项覆盖度。引擎不可用或页面结构变更时必须显式失败/待补，不能以模型臆测补齐。
...
- 部署到 Mac Mini/Hermes 后先执行 `collect_market_evidence.py --preflight --all-engines`。Ego Lite 或 Kimi WebBridge 未安装、扩展未连接或适配器未配置时，工具给出可展示的安装动作，不会降级或虚构价格。
